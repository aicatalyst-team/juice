...
<body class="h-full text-[#24170f] font-sans selection:bg-juiceOrange/30 selection:text-[#24170f] overflow-hidden flex flex-col bg-juiceDark">
    ...
    <!-- App Shell Container -->
    <div class="flex-1 flex flex-col h-full max-w-md mx-auto w-full md:max-w-7xl md:px-6 relative overflow-hidden pb-16 md:pb-0">
        
        <!-- Top Identity Area -->
        <header class="pt-6 pb-4 px-4 flex flex-col items-center justify-center shrink-0 border-b border-juiceBorder/50 md:border-b-0 md:pt-8 md:pb-6">
            <div class="relative group cursor-pointer" onclick="refreshData()">
                <div class="absolute -inset-3 rounded-full bg-gradient-to-r from-juiceOrange via-juiceLightOrange to-juiceGreen opacity-35 blur-xl group-hover:opacity-55 transition duration-500"></div>
                <!-- Large centered logo for mobile-first identity -->
                <img src="/logo.svg" alt="Juice Logo" class="relative h-32 w-32 md:h-28 md:w-28 object-contain hover:scale-105 transition-transform duration-300">
            </div>
            <h1 class="mt-2 text-xl font-display font-black tracking-tight bg-gradient-to-r from-[#24170f] via-[#FD5505] to-[#89C51F] bg-clip-text text-transparent">
                JUICE
            </h1>
            <p class="text-[10px] text-[#7c5f4a] font-bold tracking-wider uppercase mt-0.5">Negative avoidance constraints</p>
        </header>
        ...
        <section class="px-4 py-2.5 bg-white/45 backdrop-blur-md border-b border-juiceBorder shrink-0 md:rounded-lg md:border md:border-juiceBorder/80 md:p-3 md:mb-4 shadow-sm">
            <div class="flex flex-col gap-2.5 md:flex-row md:items-center md:justify-between">
                ...
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center w-full md:w-auto">
                    ...
                    <div class="relative w-full sm:w-60">
                        ...
                        <input type="text" id="searchInput" oninput="handleSearch(this.value)" placeholder="Search statements, triggers..." 
                            class="w-full pl-8 pr-3 py-1.5 bg-white/70 border border-juiceBorder focus:border-juiceOrange focus:ring-1 focus:ring-juiceOrange rounded-md text-xs text-[#24170f] placeholder-[#9a806a] outline-none transition-all">
                    </div>

                    <!-- Status Segmented Control -->
                    <div class="flex bg-white/65 p-0.5 rounded-md border border-juiceBorder text-xs font-semibold shadow-sm">
                        <button onclick="setStatusFilter('active')" id="btn-status-active" class="flex-1 sm:flex-none px-3 py-1 rounded-md text-juiceOrange bg-white border border-juiceBorder/70 shadow-sm transition-all duration-200">Active</button>
                        <button onclick="setStatusFilter('retired')" id="btn-status-retired" class="flex-1 sm:flex-none px-3 py-1 rounded-md text-[#7c5f4a] hover:text-[#24170f] transition-all duration-200">Retired</button>
                        <button onclick="setStatusFilter('all')" id="btn-status-all" class="flex-1 sm:flex-none px-3 py-1 rounded-md text-[#7c5f4a] hover:text-[#24170f] transition-all duration-200">All</button>
                    </div>
                </div>
                ...
                <div class="hidden md:flex items-center gap-2.5">
                    <!-- Count indicator -->
                    <span id="countsBadge" class="text-[11px] text-stone-400 bg-juiceCard/60 px-2.5 py-1.5 rounded-md border border-juiceBorder/50">
                        Loading...
                    </span>
                    ...
                    <div class="flex items-center gap-1 bg-juiceCard/40 pl-2 pr-1 py-1 rounded-md border border-juiceBorder">
                        <input type="text" id="newCategoryInput" placeholder="New column..." 
                            class="bg-transparent text-xs text-stone-200 placeholder-stone-500 outline-none w-24 py-0.5">
                    ...
                    <!-- AI Suggest Option -->
                    <button onclick="openSuggestModal()" class="px-3 py-1.5 bg-juiceCard hover:bg-juiceCardHover hover:text-juiceGreen border border-juiceBorder hover:border-juiceGreen/30 text-stone-300 rounded-md text-xs font-semibold flex items-center gap-1.5 transition-all shadow-sm">
                    ...
                    <!-- Refresh Button -->
                    <button onclick="refreshData()" class="p-1.5 bg-juiceCard hover:bg-juiceCardHover border border-juiceBorder hover:border-juiceBorder/80 text-stone-400 hover:text-stone-200 rounded-md transition-all" title="Refresh Dashboard">
        ...
        <main class="flex-1 overflow-x-auto overflow-y-hidden px-4 py-3 md:px-0 md:pb-6 flex gap-4 items-start" id="kanbanBoard">
            ...
            <div class="flex items-center justify-center w-full h-64 text-stone-500">
        ...
        <div class="md:hidden fixed bottom-0 left-0 right-0 bg-white/78 backdrop-blur-xl border-t border-juiceBorder/80 px-4 py-2.5 flex items-center justify-between gap-3 z-30 shadow-[0_-10px_30px_rgba(88,52,22,0.10)]">
            <button onclick="refreshData()" class="p-2.5 bg-white/90 border border-juiceBorder text-[#7c5f4a] hover:text-[#24170f] rounded-md transition-all shrink-0 shadow-sm" title="Refresh Dashboard">
                <svg class="w-4 h-4" id="refreshIconMobile" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 1121.21 15H19" />
                </svg>
            </button>
            
            <button onclick="openSuggestModal()" class="flex-1 py-2 bg-white/90 border border-juiceBorder hover:border-juiceGreen/40 text-[#4f6f1c] rounded-md text-xs font-bold flex items-center justify-center gap-1.5 transition-all shadow-sm">
            ...
            <button onclick="openCreateModal()" class="flex-1 py-2 bg-gradient-to-r from-juiceOrange to-juiceLightOrange text-white text-xs font-black rounded-md flex items-center justify-center gap-1.5 transition-all shadow-glow-orange">
    ...
    <div id="juiceModal" class="fixed inset-0 z-40 hidden flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm transition-opacity duration-300">
        ...
        <div class="bg-juiceCard border border-juiceBorder rounded-t-lg sm:rounded-lg w-full max-w-xl shadow-2xl overflow-hidden flex flex-col transform translate-y-full sm:translate-y-0 sm:scale-95 opacity-0 transition-all duration-300 max-h-[92vh] sm:max-h-[85vh]" id="modalContent">
            ...
            <div class="px-5 py-3.5 border-b border-juiceBorder/80 flex items-center justify-between bg-juiceDark/30 shrink-0">
                <h3 class="text-sm font-display font-black text-stone-100 flex items-center gap-2" id="modalTitle">
                    Create Avoidance Constraint
                </h3>
                <button onclick="closeModal()" class="text-stone-400 hover:text-stone-200 p-1 rounded-md hover:bg-juiceCardHover transition-colors">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <form id="juiceForm" onsubmit="handleFormSubmit(event)" class="p-5 space-y-4 overflow-y-auto flex-1">
                ...
                <!-- Statement -->
                <div>
                    <label class="block text-[10px] font-bold uppercase tracking-wider text-stone-400 mb-1">Statement *</label>
                    <textarea id="formStatement" required rows="3" placeholder="Define what to avoid (e.g., 'Avoid raw promises when async/await is appropriate')" 
                        class="w-full px-3 py-2 bg-juiceDark/50 border border-juiceBorder focus:border-juiceOrange focus:ring-1 focus:ring-juiceOrange rounded-md text-xs text-stone-100 placeholder-stone-600 outline-none transition-all resize-none"></textarea>
                </div>

                <!-- Category & Scope -->
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                        <label class="block text-[10px] font-bold uppercase tracking-wider text-stone-400 mb-1">Category</label>
                        <input type="text" id="formCategory" list="existingCategories" placeholder="e.g., Styling, Code Quality" 
                            class="w-full px-3 py-2 bg-juiceDark/50 border border-juiceBorder focus:border-juiceOrange focus:ring-1 focus:ring-juiceOrange rounded-md text-xs text-stone-100 placeholder-stone-600 outline-none transition-all">
                        <datalist id="existingCategories"></datalist>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold uppercase tracking-wider text-stone-400 mb-1">Scope</label>
                        <select id="formScope" onchange="handleScopeChange(this.value)" 
                            class="w-full px-3 py-2 bg-juiceDark/50 border border-juiceBorder focus:border-juiceOrange focus:ring-1 focus:ring-juiceOrange rounded-md text-xs text-stone-100 outline-none transition-all">
                ...
                <div id="scopeIdentityGroup" class="hidden p-3 bg-juiceDark/40 rounded-md border border-juiceBorder/80 space-y-2.5">
                    ...
                    <div>
                        <label class="block text-[9px] font-bold uppercase tracking-wider text-stone-500 mb-1" id="scopeKeyLabel">Scope Key</label>
                        <input type="text" id="formScopeKey" placeholder="e.g., project-name, repo-path" 
                            class="w-full px-3 py-1.5 bg-juiceDark border border-juiceBorder focus:border-juiceOrange focus:ring-1 focus:ring-juiceOrange rounded-md text-xs text-stone-200 outline-none transition-all">
                    </div>

                    <div class="grid grid-cols-3 gap-2 pt-0.5">
                        <div>
                            <label class="block text-[8px] font-bold uppercase tracking-wider text-stone-500 mb-0.5">Project</label>
                            <input type="text" id="formIdentityProject" placeholder="optional" class="w-full px-2 py-1 bg-juiceDark border border-juiceBorder rounded text-[10px] text-stone-200 outline-none">
                        </div>
                        <div>
                            <label class="block text-[8px] font-bold uppercase tracking-wider text-stone-500 mb-0.5">Repo</label>
                            <input type="text" id="formIdentityRepo" placeholder="optional" class="w-full px-2 py-1 bg-juiceDark border border-juiceBorder rounded text-[10px] text-stone-200 outline-none">
                        </div>
                        <div>
                            <label class="block text-[8px] font-bold uppercase tracking-wider text-stone-500 mb-0.5">Agent</label>
                            <input type="text" id="formIdentityAgent" placeholder="optional" class="w-full px-2 py-1 bg-juiceDark border border-juiceBorder rounded text-[10px] text-stone-200 outline-none">
                        </div>
                    </div>
                </div>
                ...
                <!-- Triggers Input -->
                <div>
                    <label class="block text-[10px] font-bold uppercase tracking-wider text-stone-400 mb-1">Triggers (comma separated)</label>
                    <input type="text" id="formTriggers" placeholder="e.g., build, deploy, test, tailwind" 
                        class="w-full px-3 py-2 bg-juiceDark/50 border border-juiceBorder focus:border-juiceOrange focus:ring-1 focus:ring-juiceOrange rounded-md text-xs text-stone-100 placeholder-stone-600 outline-none transition-all">
                    <p class="text-[9px] text-stone-500 mt-1">Triggers match context or behavior hooks to activate this juice rule.</p>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                    <div>
                        <div class="flex justify-between items-center mb-1">
                            <label class="block text-[10px] font-bold uppercase tracking-wider text-stone-400">Confidence</label>
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-1">
                            <label class="block text-[10px] font-bold uppercase tracking-wider text-stone-400">Strength</label>
                ...
                <div class="flex justify-end gap-2.5 pt-4 border-t border-juiceBorder/60">
                    <button type="button" onclick="closeModal()" class="px-3.5 py-1.5 bg-juiceDark/40 hover:bg-juiceDark text-stone-400 hover:text-stone-200 border border-juiceBorder rounded-md text-xs font-semibold transition-all">
                        Cancel
                    </button>
    ...
    <div id="suggestModal" class="fixed inset-0 z-40 hidden flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm transition-opacity duration-300">
        ...
        <div class="bg-juiceCard border border-juiceBorder rounded-t-lg sm:rounded-lg w-full max-w-lg shadow-2xl overflow-hidden flex flex-col transform translate-y-full sm:translate-y-0 sm:scale-95 opacity-0 transition-all duration-300 max-h-[92vh] sm:max-h-[85vh]" id="suggestModalContent">
            ...
            <div class="px-5 py-3.5 border-b border-juiceBorder/80 flex items-center justify-between bg-juiceDark/30 shrink-0">
                ...
                <button onclick="closeSuggestModal()" class="text-stone-400 hover:text-stone-200 p-1 rounded-md hover:bg-juiceCardHover transition-colors">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>
            ...
            <div class="p-5 space-y-4 overflow-y-auto flex-1">
                ...
                <div>
                    <label class="block text-[10px] font-bold uppercase tracking-wider text-stone-400 mb-1">Feedback / Behavior to capture *</label>
                    <textarea id="suggestFeedback" required rows="3" placeholder="e.g., 'The developer asked me to stop using inline styles and stick strictly to Tailwind classes, and always use semantic HTML.'" 
                        class="w-full px-3 py-2 bg-juiceDark/50 border border-juiceBorder focus:border-juiceGreen focus:ring-1 focus:ring-juiceGreen rounded-md text-xs text-stone-100 placeholder-stone-600 outline-none transition-all resize-none"></textarea>
                </div>

                <div class="grid grid-cols-3 gap-2">
                    <div>
                        <label class="block text-[8px] font-bold uppercase tracking-wider text-stone-500 mb-0.5">Project</label>
                        <input type="text" id="suggestProject" placeholder="optional" class="w-full px-2 py-1 bg-juiceDark border border-juiceBorder rounded text-xs text-stone-200 outline-none">
                    </div>
                    <div>
                        <label class="block text-[8px] font-bold uppercase tracking-wider text-stone-500 mb-0.5">Repo</label>
                        <input type="text" id="suggestRepo" placeholder="optional" class="w-full px-2 py-1 bg-juiceDark border border-juiceBorder rounded text-xs text-stone-200 outline-none">
                    </div>
                    <div>
                        <label class="block text-[8px] font-bold uppercase tracking-wider text-stone-500 mb-0.5">Agent</label>
                        <input type="text" id="suggestAgent" placeholder="optional" class="w-full px-2 py-1 bg-juiceDark border border-juiceBorder rounded text-xs text-stone-200 outline-none">
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-2.5 pt-2">
                    <button type="button" onclick="closeSuggestModal()" class="px-3.5 py-1.5 bg-juiceDark/40 hover:bg-juiceDark text-stone-400 hover:text-stone-200 border border-juiceBorder rounded-md text-xs font-semibold transition-all">
                        Cancel
                    </button>
                    <button type="button" onclick="submitSuggest()" class="px-4 py-1.5 bg-gradient-to-r from-juiceGreen to-emerald-500 hover:from-juiceGreen hover:to-juiceGreen text-stone-950 text-xs font-bold rounded-md shadow-glow-green transition-all transform active:scale-95">
                        Get Suggestion
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        async function refreshData() {
            ...
            try {
                ...
                document.getElementById('kanbanBoard').innerHTML = \`
                    <div class="flex flex-col items-center justify-center w-full h-64 text-stone-500 gap-2 px-4 text-center">
                        ...
                        <span class="text-xs font-bold text-stone-400">Failed to sync with backend server</span>
                        <button onclick="refreshData()" class="mt-1 px-3 py-1.5 bg-juiceCard hover:bg-juiceCardHover text-stone-200 text-xs rounded-md font-medium transition-colors">
                            Retry Connection
                        </button>
                    </div>
                \`;
        ...
        function renderBoard() {
                    ...
                    tabsHtml += \`
                        <button onclick="setMobileCategory('\${escapeHtml(cat)}')" class="px-3 py-1 rounded-md text-xs font-bold whitespace-nowrap border transition-all shrink-0 \${
                            isActive 
                                ...
                                : 'bg-juiceCard border-juiceBorder text-stone-400 hover:text-stone-200'
                        }">
                            \${escapeHtml(displayName)}
                            <span class="ml-1 text-[9px] px-1 py-0.5 rounded-full bg-juiceDark/40 text-stone-500">\${catJuicesCount}</span>
                        </button>
                    \`;
                });
            ...
            if (filteredJuices.length === 0 && categories.length === 0) {
                html = \`
                    <div class="flex flex-col items-center justify-center w-full py-16 text-stone-500 gap-2 px-4 text-center">
                        <svg class="w-10 h-10 text-stone-800" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
                        </svg>
                        <span class="text-xs font-bold text-stone-400">No constraints found</span>
                        <p class="text-[10px] text-stone-600">Try adjusting filters or create a new avoidance constraint.</p>
                    </div>
                \`;
            }
        ...
        function renderColumnHtml(displayName, categoryValue, juices, isMobile) {
            ...
            if (juices.length === 0) {
                cardsHtml = \`
                    <div class="flex-1 flex flex-col items-center justify-center py-12 border border-dashed border-juiceBorder rounded-md text-stone-600 text-xs">
            ...
            const columnWidthClass = isMobile ? 'w-full' : 'w-80 shrink-0';
            const maxHeightClass = isMobile ? 'max-h-[calc(100vh-270px)]' : 'max-h-[calc(100vh-220px)]';
                ...
                <div class="\${columnWidthClass} flex flex-col \${maxHeightClass} bg-juiceCard/20 border border-juiceBorder md:border-juiceBorder/80 rounded-md p-3.5 transition-all duration-300"
                    ...
                    <div class="hidden md:flex items-center justify-between mb-3 shrink-0">
                        <div class="flex items-center gap-1.5">
                            <h4 class="text-xs font-display font-black text-stone-200 truncate max-w-[160px]">\${escapedDisplayName}</h4>
                            <span class="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-juiceDark text-stone-400">\${juices.length}</span>
                        </div>
                        <button data-category="\${escapedCategory}" onclick="openCreateModal(this.dataset.category || '')" class="p-1 hover:bg-juiceCardHover text-stone-400 hover:text-juiceOrange rounded-md transition-colors" title="Add to this category">
                            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        function renderCardHtml(j) {
            ...
            if (j.triggers && j.triggers.length > 0) {
                ...
                shownTriggers.forEach(t => {
                    triggersHtml += \`<span class="text-[10px] font-medium px-1.5 py-0.5 rounded bg-juiceDark text-stone-400 border border-juiceBorder truncate max-w-[90px]">\${escapeHtml(t)}</span>\`;
                });
                if (overflowCount > 0) {
                    triggersHtml += \`<span class="text-[10px] font-medium px-1.5 py-0.5 rounded bg-juiceDark text-stone-500 border border-juiceBorder font-bold">+\${overflowCount}</span>\`;
                }
            }
            ...
            if (isRetired) {
                actionsHtml = \`
                    <button data-id="\${id}" onclick="restoreJuice(this.dataset.id)" class="p-1 text-stone-500 hover:text-juiceGreen hover:bg-juiceDark rounded transition-colors" title="Restore rule">
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 1121.21 15H19" />
                        </svg>
                    </button>
                ...
                actionsHtml = \`
                    <button data-id="\${id}" onclick="openEditModal(this.dataset.id)" class="p-1 text-stone-500 hover:text-stone-200 hover:bg-juiceDark rounded transition-colors" title="Edit">
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                        </svg>
                    </button>
                    <button data-id="\${id}" onclick="retireJuice(this.dataset.id)" class="p-1 text-stone-500 hover:text-juiceOrange hover:bg-juiceDark rounded transition-colors" title="Retire">
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            const retiredLabel = isRetired ? \`<span class="text-[8px] font-bold uppercase tracking-wider px-1 py-0.5 rounded bg-juiceDark text-stone-500 border border-juiceBorder shrink-0">Retired</span>\` : '';
            ...
            return \`
                <div class="bg-white/72 hover:bg-white/90 rounded-md p-3 shadow-[0_8px_24px_rgba(88,52,22,0.08)] hover:shadow-[0_10px_28px_rgba(88,52,22,0.12)] transition-all duration-200 cursor-grab active:cursor-grabbing relative group/card \${retiredClass}"
                    ...
                    <div class="flex items-center justify-between mb-2">
                        <div class="text-[10px] text-[#8a705d] font-medium flex flex-wrap items-center gap-1.5 overflow-hidden">
                            \${retiredLabel}
                    ...
                    <!-- Statement Text -->
                    <p class="text-[13px] leading-relaxed text-[#24170f] font-medium mb-3 break-words line-clamp-4 hover:line-clamp-none transition-all duration-300">
                        \${statement}
                    </p>
                    ...
                    <div class="flex items-center justify-between gap-2 flex-wrap">
                        ...
                        <div class="flex items-center gap-1.5 shrink-0">
                            <span class="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[#f3f8df] text-[#4f6f1c] border border-[#dce8ba]" title="Confidence">C \${normalizedScore(j.confidence).toFixed(2)}</span>
                            <span class="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[#fff0e6] text-juiceOrange border border-[#ffd3b8]" title="Strength">S \${normalizedScore(j.strength).toFixed(2)}</span>
                        </div>
                    </div>
                </div>
            \`;
